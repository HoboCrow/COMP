...
is_vardec* insert_integer_dec(char* id, int nline, int ncol) {
    is_vardec* iv = (is_vardec*)malloc(sizeof(is_vardec));
    is_integer_dec* iid = (is_integer_dec*)malloc(sizeof(is_integer_dec));

    iid->id =
        (char*)strdup(id); /* Por precaucao. Seria apenas necessario copiar o
                              ponteiro, pois o strdup ja foi feito atras*/
    iid->nline = nline;
    iid->ncol = ncol;
    iv->disc_d = d_integer;
...
is_vardec* insert_character_dec(char* id, int nline, int ncol) {
    is_vardec* iv = (is_vardec*)malloc(sizeof(is_vardec));
...
is_vardec* insert_double_dec(char* id, int nline, int ncol) {
    is_vardec* iv = (is_vardec*)malloc(sizeof(is_vardec));
...
is_statement_list* insert_statement_list(is_statement_list* head,
                                         is_statement* is) {
    is_statement_list* isl =
        (is_statement_list*)malloc(sizeof(is_statement_list));
    is_statement_list* tmp;

    isl->val = is;
    isl->next = NULL;

    if (head == NULL) return isl;
...
is_statement* insert_write_statement(char* id, int nline, int ncol) {
    is_statement* is = (is_statement*)malloc(sizeof(is_statement));
    is_write_statement* iws =
        (is_write_statement*)malloc(sizeof(is_write_statement));
